...
| [`isaacsim/`](isaacsim/README.md) | the contested-terrain range as compacted USD scene layers plus a shipped heightmap; NVIDIA and Poly Haven content is fetched locally by `isaacsim/tools/fetch_assets.py`, the terrain is rebuilt by `isaacsim/tools/build_terrain.py`, and `isaacsim/tools/range_doe.py` emits DOE variant layers (obstacle density, slope scale, PhysX friction/restitution, multi-robot) | curated model files | Isaac Sim required; nothing here was run in Isaac Sim by the release preparation unless `isaacsim/README.md` says so |
...
<!-- TODO(range): reconcile with isaacsim/README.md -->
...
- None of the case studies ran on hardware, in Gazebo or in Isaac Sim; each directory's README states what stands in for the real pipeline (a synthetic detector and world, an independent planner implementation, and so on).
...
- The paper's own headline numbers — the sensor-suite enumeration counts, the multi-robot robustness values, the range's slope and coverage bounds — are not reproduced by the shipped code, and each directory's README states what it produces instead.

...
(again, prefix with `env -u PYTHONPATH` if you have ROS 2 sourced). The suite is 55 passed and
2 skipped in 2.6 s — the two skips are the Gurobi solve, which needs the `milp` extra, and the
real-`verifyta` check, which runs as a 56th test when `UPPAAL_HOME` points at an UPPAAL install.
...
| `pytest -q` over the whole suite | 55 passed, 2 skipped in 2.6 s; 56 passed, 1 skipped with `UPPAAL_HOME` set |

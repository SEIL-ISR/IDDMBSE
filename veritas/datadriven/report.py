...
def group_index(design, environment):
    """(design, environment) pairs to a group index array and the list of group labels."""
    labels = np.array([str(d) + " | " + str(e) for d, e in zip(design, environment)], dtype=object)
    uniq, idx = np.unique(labels.astype(str), return_inverse=True)
...
def summarise(success, design, environment, rho=None, delta=0.05, target=0.05, level=0.05):
    ...
    idx, groups = group_index(design, environment)
    n_groups = len(groups)
...
def read_database(db_path, metric="robustness"):
    c = pa.read_campaign(db_path)
    rho = pa.read_metric(db_path, c["trial_id"], name=metric)
    return c["success"], c["design_name"], c["environment_name"], rho
...
def figure(s, out_dir):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([d + "\n" + e for d, e in s["groups"]], fontsize=8)
...
def main():
    ...
    if a.db:
        success, design, environment, rho = read_database(a.db, a.metric)
    else:
    ...
    s = summarise(success, design, environment, rho=rho,

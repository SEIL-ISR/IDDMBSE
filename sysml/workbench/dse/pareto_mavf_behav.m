...
% bag_oracle =rosbag('warty_baseconfig_hard_oracle_1_2023-10-02-17-38-26.bag')
% 
...
% 
% title('Trajectory Plots for the Lidar-only Sensor Configurations in the HARD Test Case')
...
for k = 1 : length(theFiles)
    ...
    xPoints = cellfun(@(m) double(m.Pose.Position.X),runs_pose_msgs);
    yPoints = cellfun(@(m) double(m.Pose.Position.Y),runs_pose_msgs);
    zPoints = cellfun(@(m) double(m.Pose.Position.Z),runs_pose_msgs);
    plot3(xPoints,yPoints,zPoints,'LineWidth',1)
    hold on
end
...
deviations = zeros(length(theFiles),1);
...
for k = 1 : length(theFiles)
    ...
    deviations(k) = arclen/arclen_oracle;

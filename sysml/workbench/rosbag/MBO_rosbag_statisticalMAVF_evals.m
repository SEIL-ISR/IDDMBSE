...
% GOALS = [
%     dict(x=6, y=3.5, z=1, w=0.5),
%     dict(x=7, y=4.2, z=1, w=1.5),
%     dict(x=5, y=3.0, z=1, w=0.1),
% ]


%%
% Specify the directory path
directory = 'SensorBags/';
...
% Exclude '.' and '..' folders (current directory and parent directory)
...
% Print all bags in the all folders
clear all
clc
%close all
% Specify the directory path
main_directory = 'SensorBags/';

% Get a list of all folders in the main directory
folders = dir(main_directory);
folders = folders([folders.isdir]);  % Only keep directories, exclude '.' and '..'
folder_names = {folders.name};       % Extract the names of the folders
folder_names = folder_names(~ismember(folder_names, {'.', '..'}));

% Iterate through each folder
for i = 1:length(folder_names)
    current_folder = fullfile(main_directory, folder_names{i});
    
    % List all files in the current folder
    files = dir(fullfile(current_folder, '*.bag'));
    file_names = {files.name};  % Extract the names of the files
    
    % Display the folder name
    disp(['Folder: ' folder_names{i}]);
    
    % Iterate through each CSV file in the folder
    %figure(i); 
    figure(1); 
    plot(6, 3.5, 'v', 'MarkerSize',20, "MarkerEdgeColor","#7E2F8E" )
    hold on
    plot(7, 4.2, 'v', 'MarkerSize',20, "MarkerEdgeColor","#77AC30")
    plot(5, 3, 'v', 'MarkerSize',20, "MarkerEdgeColor","#4DBEEE")
    title(['Trajectory Plot of the Runs; Design ', folder_names{i}]);
    xlabel('X-axis');
    ylabel('Y-axis');
    %zlabel('Z-axis');
    xlim([-2 10])
    ...
    for j = 1:length(file_names)
        ...
        file_path = fullfile(current_folder, file_names{j});
        
        % Load and process the CSV file (example: just displaying its name)
        disp(['   Bag File: ' file_names{j}]);
        
        % Your bag processing code goes here       

        run_bag = rosbag(file_path);
        runs_poseTopics = select(run_bag, 'Topic','/odometry/filtered');
        runs_pose_bags(j) = runs_poseTopics;    
        runs_pose_msgs= readMessages(runs_pose_bags(j),'DataFormat','struct');
        ...
        %hold on
        plot(xPoints,yPoints,'LineWidth',1.5)                
        
        

    end
    pathlen = zeros(length(files),1);
    TTCs = zeros(length(files),1);
    CEGs = zeros(length(files),1);
    ...
    disp(['Folder: ' folder_names{i}]);
    disp(['File: ' file_names{j}]);
    metrics= horzcat(TTCs, pathlen)

    legend('goal 1',  'goal 2', 'goal 3', 'Trajectory 1', 'Trajectory 2', 'Trajectory 3')
    hold off
    %saveas(gcf, ['figure_', num2str(i), '.png']);
end

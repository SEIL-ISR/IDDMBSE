...
Six directories mirror the paper's Section IV. A1, A2 and A3 are the published
demonstrations of Section IV-A and point into the tool directories at the repository
root rather than carrying their own algorithm code. B1, B2 and B3 are the demonstrations
released with the paper, Section IV-B. A1 reruns a recorded lab campaign through the
Python and Julia code now in `trades-x/`; A3 reruns a vendored demo unmodified. A2, B2
and B3 carry no prior implementation in this repository — each says in its own first
paragraph that it was built for this release, to the paper's description, and each
reports what it actually produced rather than the paper's own numbers.

| id | paper | directory | run | needs | what it produces | checked here (2026-09-22) |
|---|---|---|---|---|---|---|
| A1 | IV-A.1, sensor-suite selection | [`A1-sensor-suite-selection/`](A1-sensor-suite-selection/README.md) | `./run.sh` | uv (the `trades-x/` project; no separate setup step) | the sensor-suite Pareto filter and MAVF ranking, reproducing the recorded 4095-design run and comparing it against the paper's own text | `run_case_study.py` prints `GATE G2: pass`; the recorded 4095-design run gives 96 Pareto designs (standard and MATLAB-compatible filters agree), the full 8191-design enumeration gives 120, against the paper's stated 92 |
...
| A3 | IV-A.3, behavior-tree verification | [`A3-behavior-tree-verification/`](A3-behavior-tree-verification/README.md) | `./run.sh` | uv (the `veritas/` project); UPPAAL only to check the written model, not to run the demo | an UPPAAL network of timed automata (`BT_converted.xml`) composed from a behavior tree, for model checking and control synthesis in UPPAAL (not run here) | the demo's output is byte-identical to the vendored upstream script's own output (md5 `9f1da760f65674818d156546092e6118`); `pytest tests/test_bt2automata.py tests/test_tbt_monitor.py` is 4 passed in 0.26 s (both from `veritas/README.md`) |
| B1 | IV-B.1, the contested-terrain range | [`B1-contested-terrain-range/`](B1-contested-terrain-range/README.md) | the `isaacsim/tools` quick start (fetch, build the terrain, open the scene, `range_doe.py` for a design point) | Isaac Sim, a local install with a GPU, to open the scene; `uv` for the tools | the range: compacted scene layers, a heightmap-rebuilt terrain, three design-of-experiments layers, two showcases (24 MB shipped; ~4.9 GiB of NVIDIA and Poly Haven content fetched locally) | 22 tests pass in `isaacsim/tools`; one headless load of the base scene and one DOE layer in Isaac Sim 6.0.1: 0 unresolved references; no campaign run in the range |
